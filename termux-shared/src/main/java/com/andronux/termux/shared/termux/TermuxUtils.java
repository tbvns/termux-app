...
import com.andronux.termux.shared.reflection.ReflectionUtils;
...
import com.andronux.termux.shared.termux.TermuxConstants.TERMUX_APP;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `com.andronux.termux`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.andronux.termux/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="com.andronux.termux" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(com.andronux.termux.shared.R.raw.apt_info_script);
        try {
